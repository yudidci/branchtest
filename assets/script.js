...
//btn update warga
$('#btn-updatewarga').on('click', function (e) {
    updateWargaList();
    $(".updateWargaList").css("display", "block");
})
...
function updateWargaList() {
    $.ajax({
        ...
        success: function (result) {
            let movies = result.result;
            $('#updatewarga-list').html('');
            $("#updatewarga-list").append(
                "<div class='updateWargaList'><h1>NIK Detail</h1>\
                    <table>\
                        <thead>\
                            <tr>\
                           \
            ...
            if (result.status = 200) {
                $.each(movies, function (i, data) {
                    $('.updateWargaList').find("tbody").append(`
